    ...
    for (double i = x.min; i < x.max; i+=_h) {
        ...
        for (double j = y.min; j < y.max; j+=_h) {
            ...
            for (double k = z.min; k < z.max; k+=_h) {
                ...
                temp[_nPoints] = {i, j, k};
                //..add it to the surface if it's inside the narrow-band domain..
                // double dist = phi(i,j,k);
                // if (dist > -_delta && dist < _delta) {
                //     temp_s[spoints] = {i,j,k};
                //     ++spoints;
                // }
                //..create corresponding neighbour
                _neighbour[_nPoints].point[0] = _nPoints;   // itself
                _neighbour[_nPoints].point[1] = std::max((_nPoints + 1) * top, -1);
                _neighbour[_nPoints].point[2] = std::max(abs(_nPoints - 1) * bottom, -1);
                _neighbour[_nPoints].point[3] = std::max((_nPoints + zPoints) * behind, -1);
                _neighbour[_nPoints].point[4] = std::max(abs(_nPoints - zPoints) * front, -1);
                _neighbour[_nPoints].point[5] = std::max((_nPoints + yPoints * zPoints) * right, -1);
                _neighbour[_nPoints].point[6] = std::max(abs(_nPoints - yPoints * zPoints) * left, -1);
                ++_nPoints;
            }
        }
    }
...
void Grid3::saveRelations(std::string filename) {
    ...
    // points around
    // 1 row per point to _neighbour indexes: itself top bottom behind
    // front right left
...
void Grid3::createProjection() {
    ...
    for (int i = 0; i < _nPoints; ++i) {
        ...
        //(A1) find the points (actually, only the left-front-bottom)
